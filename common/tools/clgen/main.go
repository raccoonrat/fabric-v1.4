...
func generatePeerOrg(baseDir string, orgSpec OrgSpec) {
	...
	err = msp.GenerateVerifyingMSP(mspDir, signKGC, tlsCA, orgSpec.EnableNodeOUs)
	if err != nil {
...
func generateNodes(baseDir string, nodes []NodeSpec, signKGC *kgc.KGC, tlsCA *ca.CA, nodeType int, nodeOUs bool) {

	for _, node := range nodes {
		nodeDir := filepath.Join(baseDir, node.CommonName)
		if _, err := os.Stat(nodeDir); os.IsNotExist(err) {
			err := msp.GenerateLocalMSP(nodeDir, node.CommonName, node.SANS, signKGC, tlsCA, nodeType, nodeOUs)
			if err != nil {
